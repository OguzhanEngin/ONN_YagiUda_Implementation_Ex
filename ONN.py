...
from utils.opt_func_matlab import AntennaFunction_v2
...
n_dimensions = 11 # n_dimensions of optimization benchmark function
s_init = 2*n_dimensions # number of initial samples (initial dataset)
num_candidates = 2*n_dimensions # number of candidate solutions (it has to be smaller than s_init)
...
#------------------------------------------------------------------------------+
#
#   Cost Function, NOTE: Did a modification here to change the function to YagiUda
#
#------------------------------------------------------------------------------+

yagiuda_org_func = AntennaFunction_v2()
dimension_names = yagiuda_org_func.x_names
def yagiuda_func(x):
    x_ser = pd.Series(index=dimension_names, data=x)
    obj_val = yagiuda_org_func.calculate_val(x_ser)
    return -obj_val

func = yagiuda_func
var_bounds = [yagiuda_org_func.x_min, yagiuda_org_func.x_max]
